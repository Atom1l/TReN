...
// 🟢 1. ฟังก์ชันแปลภาษาอัจฉริยะ (พร้อมระบบ Smart Retry สำหรับคำสั้นๆ และตัวพิมพ์ใหญ่)
const translateText = async (text: string, targetLang: string) => {
  ...
  // Trick: ถ้าแปลรอบแรกแล้วได้คำเดิม และเป็นคำสั้น (เช่น "Arts", "Science") ให้ลองแปลแบบพิมพ์เล็ก
...
// 🟢 2. แยก ShowcaseCard เป็น Component เดี่ยว เพื่อให้ตอบสนองการแปลภาษาทันที
const ShowcaseCard: React.FC<{ 
  showcase: ShowcaseData; 
  onClick: () => void; // จะลบทิ้งหรือเก็บไว้ก็ได้
  normalizeTag: (rawTag: string) => string; 
}> = ({ showcase, normalizeTag }) => {
  const { t, language } = useLanguage();
  const navigate = useNavigate(); // 👈 เรียกใช้งาน navigate ตรงนี้ได้เลย
  ...
  useEffect(() => {
    const autoTranslateCard = async () => {
      ...
      try {
        const newTitle = await translateText(showcase.title, language);
  ...
  return (
    <div 
      // 💡 เปลี่ยนจากเปิด Modal เป็นสั่ง navigate ไปหน้า Detail ทันที
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {/* 🟢 เรียกใช้ ShowcaseCard แบบใหม่ */}
              {currentItems.map(sc => (
                <ShowcaseCard 
                  key={sc.id} 
                  showcase={sc} 
                  onClick={() => setViewModal({ isOpen: true, showcase: sc })}
