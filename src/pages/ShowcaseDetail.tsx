...
// 💡 นำเข้า Constants จังหวัดเพื่อดึงชื่อภาษาไทย
...
const ShowcaseDetail = () => {
  ...
  const [relatedShowcases, setRelatedShowcases] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchShowcaseData = async () => {
      try {
        ...
        if (showcaseData.status !== 'published') {
          const isAdmin = ['admin', 'developer', 'co_admin', 'co-admin'].includes(currentUserRole);
          const isAuthor = user && user.id === showcaseData.author_id;
        ...
        if (showcaseData?.author_id) {
          const { data: authorData } = await supabase
            .from('user')
            .select('id, first_name, last_name, bio, profilepic')
            .eq('id', showcaseData.author_id)
            .single();
          setPrimaryAuthor(authorData);
        }

        if (showcaseData?.tag) {
          const mainTag = showcaseData.tag.split(',')[0].trim();
    ...
    if (id) fetchShowcaseData();
  }, [id, navigate, t]);

  // 🟢 2. Script เพื่อเพิ่มป้ายกำกับ "คลิกเพื่อดูภาพเต็ม" ในภาพที่เป็น Content อัตโนมัติ
  useEffect(() => {
  ...
  }, [translatedContent, showcase, t]); 

  // useEffect สำหรับดักจับการเปลี่ยนภาษา
  useEffect(() => {
    const autoTranslate = async () => {
      if (!showcase) return;

      setTranslatedTitle(showcase.title);
      setTranslatedContent(showcase.description);
  ...
  const tags = showcase?.tag ? showcase.tag.split(',').map((t: string) => t.trim()).filter(Boolean) : [];

  let parsedLinks: { title: string, url: string }[] = [];
  if (showcase && showcase['Link to work']) {
    try {
  ...
  try {
    if (showcase && showcase.author_data) {
  ...
  // 💡 ดึงค่าจังหวัดจาก Constants มาแสดงเป็นภาษาไทยให้สวยงาม
  const getProvinceLabel = (value: string) => {
    if (!value) return '';
    const prov = THAI_PROVINCES.find((p: { value: string; label: string }) => p.value === value);
    return prov ? prov.label : value;
  };
  ...
  const shareToX = () => {
    const currentUrl = encodeURIComponent(window.location.href);
    const postTitle = encodeURIComponent(showcase?.title || 'TReN Showcase');
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: showcase?.title || 'TReN Showcase',
          text: `อ่านผลงาน: ${showcase?.title}\nบน TReN ได้ที่นี่\n`,
          url: currentUrl,
        });
      } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12">
        ...
        {showcase.status !== 'published' && (
          <div className={`px-4 py-3 rounded-xl mt-5 mb-6 flex items-center gap-3 border ${
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={`w-6 h-6 flex-shrink-0 ${showcase.status === 'rejected' ? 'text-red-600' : 'text-yellow-600'}`}>
              <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-5a.75.75 0 01.75.75v4.5a.75.75 0 01-1.5 0v-4.5A.75.75 0 0110 5zm0 10a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" />
            </svg>
            <div className="flex flex-col">
              <span className="font-medium">
                {t('preview_warning') || 'คุณกำลังดูตัวอย่าง (Preview) สถานะปัจจุบัน:'} <strong className="capitalize">{translateStatus(showcase.status)}</strong> 
              </span>
              {showcase.status === 'rejected' && showcase.rejection_reason && (
                <span className="text-sm mt-1 text-red-700 font-semibold opacity-90">
                  เหตุผล: {showcase.rejection_reason}
                </span>
              )}
            </div>
          </div>
        ...
        {showcase.thumbnail_url && (
          <div 
            ...
            onClick={() => setZoomedImage(showcase.thumbnail_url)}
          >
            <img 
        ...
        <div 
          ...
          dangerouslySetInnerHTML={{ __html: translatedContent || showcase.description || '' }}
        />
