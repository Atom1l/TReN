...
// 🟢 1. ฟังก์ชันแปลภาษาอัจฉริยะ (พร้อมระบบ Smart Retry สำหรับคำสั้นๆ)
const translateText = async (text: string, targetLang: string) => {
  ...
  // Trick แก้ปัญหาคำสั้นเฉพาะที่ Google ไม่ยอมแปล (เช่น "Arts")
...
// 🟢 2. แยกฟังก์ชันดึงไอคอนออกมาไว้ข้างนอก เพื่อให้ใช้ร่วมกันได้
const getResourceIcon = (type: string) => {
  switch (type) {
    case 'document':
      return (
...
// 🟢 3. แยก Component การ์ดออกมา เพื่อให้จัดการสถานะแปลภาษาของตัวเองได้ 100%
const ResourceCard: React.FC<{ res: ResourceData; onClick: () => void }> = ({ res, onClick }) => {
  const { t, language } = useLanguage();
  const [translatedTitle, setTranslatedTitle] = useState(res.title);
  const [translatedDesc, setTranslatedDesc] = useState(res.description || '');
    ...
    const autoTranslateCard = async () => {
      ...
      try {
        const [newTitle, newDesc] = await Promise.all([
          translateText(res.title, language),
          translateText(res.description || '', language)
        ]);

        setTranslatedTitle(newTitle);
        setTranslatedDesc(newDesc);
      } catch (err) {
        console.error("Resource card translate error:", err);
      } finally {
    ...
    <div className="min-h-screen bg-white pb-24 border-t border-slate-100">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12">
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {/* 🟢 เรียกใช้ Component ResourceCard ที่เราแยกออกมา */}
              {currentItems.map((res) => (
                <ResourceCard 
                  key={res.id} 
                  res={res} 
