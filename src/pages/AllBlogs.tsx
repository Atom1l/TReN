...
// 🟢 1. ฟังก์ชันแปลภาษาอัจฉริยะแบบนับสัดส่วนตัวอักษรไทย vs อังกฤษ ป้องกันการเข้าใจผิด
// 🟢 ฟังก์ชันแปลภาษาฉบับอัปเกรด: เพิ่มระบบ Retry อัตโนมัติสำหรับคำสั้น/คำขึ้นต้นด้วยตัวพิมพ์ใหญ่
const translateText = async (text: string, targetLang: string) => {
  if (!text || !text.trim() || text === '-') return text;
  
  // 1. ทำความสะอาดแท็ก HTML และนับสัดส่วนภาษา
  ...
  // ฟังก์ชันยิง API แปลภาษา (แยกเป็นตัวย่อยเพื่อเรียกซ้ำได้)
  ...
  // 2. สั่งแปลรอบแรกตามปกติ
  let result = await fetchTranslate(text);

  // 🟢 3. TRICK แก้ปัญหาคำว่า "Arts":
  // ถ้าแปลรอบแรกแล้ว Google ยังคืนค่าเดิมกลับมา และเป็นคำศัพท์สั้นๆ (ไม่เกิน 15 ตัวอักษร)
  // ให้ลองแปลงเป็น "ตัวพิมพ์เล็กทั้งหมด" (lowercase) แล้วส่งไปแปลใหม่อีกรอบ!
  if (result === text && targetLang === 'th' && !isThaiArticle && text.length <= 15) {
    const lowerResult = await fetchTranslate(text.toLowerCase());
    
    // ถ้าแปลแบบ lowercase แล้วได้คำไทยกลับมา (ค่าไม่เท่ากับอังกฤษพิมพ์เล็ก) ให้ใช้คำไทยนั้นเลย!
...
// ฟังก์ชันสำหรับตัดแท็ก HTML ออกจากข้อความให้เหลือแต่ตัวหนังสือ
const stripHtml = (html: string) => {
  const doc = new DOMParser().parseFromString(html, 'text/html');
  return doc.body.textContent || "";
};

// 🟢 2. แยก BlogCard ออกมาเป็น Component เดี่ยว เพื่อให้จัดการเปลี่ยนภาษาและ State ได้เอง 100%
const BlogCard: React.FC<{ 
  blog: BlogData; 
  onClick: () => void; 
  normalizeTag: (rawTag: string) => string; 
}> = ({ blog, onClick, normalizeTag }) => {
  const { t, language } = useLanguage();
  ...
  // 🟢 3. useEffect สำหรับแปลภาษาเนื้อหาภายใน BlogCard
  useEffect(() => {
    const autoTranslateCard = async () => {
      const rawSnippet = stripHtml(blog.content) || '';
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-1">
        {/* แสดงชื่อบทความที่แปลแล้ว พร้อม Badge Loading เล็กๆ ด้านบน */}
        <div className="mb-2">
          {isTranslating && (
          ...
          <h3 className="text-[#1e3a8a] text-xl font-bold line-clamp-2 leading-tight">{translatedTitle || blog.title}</h3>
        </div>
        ...
        {/* แสดงเนื้อหาย่อที่ผ่านการตัดแท็ก HTML และแปลภาษาแล้ว */}
        <p className="text-slate-500 text-sm line-clamp-3 mb-6 flex-1 leading-relaxed">
          {translatedSnippet || stripHtml(blog.content) || t('no_bio') || 'No bio available'}
        </p>
