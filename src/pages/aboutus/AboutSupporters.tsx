        ...
        <section className="mb-20">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {sponsors.map((sponsor, index) => (
              <div 
                key={index} 
                className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-200 hover:shadow-md hover:border-blue-200 transition-all duration-300 flex flex-col items-center text-center group"
              >
                {/* Logo Image Placeholder */}
                <div className="w-full h-32 md:h-40 bg-slate-50 rounded-2xl flex items-center justify-center p-4 mb-6 group-hover:bg-blue-50/50 transition-colors">
                  {sponsor.logoUrl ? (
                ...
                {/* Partner Name */}
                <h3 className="text-xl md:text-2xl font-bold text-[#1e3a8a] leading-snug mt-auto">
                  {sponsor.name}
                </h3>
              </div>
            ))}
