...
const translateText = async (text: string, targetLang: string) => {
  ...
  // เช็คว่าในข้อความเดิมมี "ตัวอักษรภาษาไทย" อยู่หรือไม่
  const hasThai = /[\u0E00-\u0E7F]/.test(text);
  
  // ดักทางป้องกันการแปลมั่วหรือแปลซ้ำภาษาเดิม
  if (targetLang === 'th' && hasThai) return text;
  if (targetLang === 'en' && !hasThai) return text;

  // ล็อคภาษาต้นทาง (sl) เพื่อไม่ให้ Google ใช้ auto เดาภาษาผิด
...
const EventDetail = () => {
  ...
  // 🟢 3. useEffect ดักจับการเปลี่ยนภาษา เมื่อกดสลับที่ Footer
  useEffect(() => {
    const autoTranslate = async () => {
      ...
      try {
        // สั่งแปลพร้อมกันทั้ง ชื่อกิจกรรม, สถานที่ และ เนื้อหาสรุป
  ...
  return (
    <div className="min-h-screen bg-white pb-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12">
        ...
        {/* 🟢 4. Title & Info ที่แปลภาษาแล้ว พร้อมป้ายสถานะ Loading */}
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#1e3a8a] leading-tight mt-4 break-words">
        ...
        {/* 🟢 5. About this Event ที่แปลภาษาแล้ว */}
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-[#1e3a8a] mb-4">{t('about_event') || 'About this Event'}</h2>
          <p className="text-slate-700 leading-relaxed text-lg whitespace-pre-wrap break-words">
