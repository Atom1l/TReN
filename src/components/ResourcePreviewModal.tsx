...
// 🟢 1. ฟังก์ชันแปลภาษาฉบับนับสัดส่วนตัวอักษรไทย vs อังกฤษ ป้องกันการเข้าใจผิด
...
const ResourcePreviewModal: React.FC<ResourcePreviewModalProps> = ({ isOpen, onClose, resource }) => {
  ...
  useEffect(() => {
    const autoTranslate = async () => {
      ...
      setTranslatedDesc(resource.description || '');
      setIsTranslating(true);

      try {
        // สั่งแปลพร้อมกันทั้ง ชื่อทรัพยากร และ คำอธิบาย
        const [newTitle, newDesc] = await Promise.all([
          translateText(resource.title, language),
          translateText(resource.description || '', language)
        ]);

        setTranslatedTitle(newTitle);
        setTranslatedDesc(newDesc);
      } catch (err) {
        console.error("Resource modal translate error:", err);
      } finally {
        ...
        <div className="w-full md:w-1/2 p-6 sm:p-10 flex flex-col justify-center overflow-y-auto custom-scrollbar">
          <div className="mb-4">
             ...
             <h2 className="text-2xl sm:text-3xl font-bold text-[#1e3a8a] mb-1 leading-tight">
               {translatedTitle || resource.title}
             </h2>
          ...
          {/* 🟢 5. แสดงผลคำอธิบายที่แปลภาษาแล้ว */}
          <div className="text-slate-600 text-sm sm:text-base mb-6 border-t border-slate-100 pt-4 leading-relaxed line-clamp-[8] overflow-y-auto break-words custom-scrollbar">
            {translatedDesc || resource.description || (t('no_description_provided') || 'ไม่มีคำอธิบายสำหรับทรัพยากรนี้')}
          </div>
